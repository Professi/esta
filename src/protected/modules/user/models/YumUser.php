...
//		}
    ...
    public function hasRole($role_title) {
        ...
        foreach ($roles as $role)
            if ((is_numeric($role) && $role == $role_title) || ($role->id == $role_title || $role->title == $role_title))
                return true;
    ...
    public function register($username = null, $password = null, $profile = null, $salt = null) {
        ...
        // registration
        if (Yum::hasModule('avatar') && Yum::module('avatar')->enableGravatar && Yum::module('avatar')->enableGravatarDefault)
            ...
            if (Yum::hasModule('role') && Yum::hasModule('registration'))
                foreach (Yum::module('registration')->defaultRoles as $role)
                    Yii::app()->db->createCommand(sprintf(
                                    'insert into %s (user_id, role_id) values(%s, %s)', Yum::module('role')->userRoleTable, $this->id, $role))->execute();
                ...
                if ($user->activationKey == $key) {
                    ...
                    if ($user->save(false, array('activationKey', 'status'))) {
                        ...
                        if (Yum::hasModule('messages') && Yum::module('registration')->enableActivationConfirmation
                            ...
                            YumMessage::write($user, 1, Yum::t('Your activation succeeded'), strtr(
    ...
    public function getAvatar($thumb = false) {
        if (Yum::hasModule('avatar') && $this->profile) {
            ...
            if (Yum::module('avatar')->enableGravatar && $this->avatar == 'gravatar')
                return CHtml::image(
                                'http://www.gravatar.com/avatar/' . $this->getGravatarHash(), Yum::t('Avatar image'), $options);
            ...
            if (isset($this->avatar) && $this->avatar)
                ...
                $return .= CHtml::image(Yii::app()->getAssetManager()->publish(
                                        Yii::getPathOfAlias('YumAssets.images') . ($thumb ? '/no_avatar_available_thumb.jpg' : '/no_avatar_available.jpg'), Yum::t('No image available'), $options));
            $return .= '</div><!-- avatar -->';
            return $return;
        }
    }
