<!DOCTYPE html>
<head>
	<meta charset="UTF-8">
	
	<meta name="viewport" content="width=device-width">
	<!-- Foundation Styles -->
	<link rel="stylesheet" href="css/foundation.min.css">
	<!-- Symbole -->
	<link rel="stylesheet" href="css/icons.css">
	<!-- Overrides -->
	<link rel="stylesheet" href="css/app.css">
	
	<title>Elternsprechtag - Ihre Termine</title>
	
	<script src="js/modernizr.foundation.js"></script>
</head>
<body class="off-canvas hide-extras">
	<div class="wrapper">
		<!-- HEADER -->
		<div class="row contain-to-grid">
			<div class="five mobile-four columns offset-by-one">
				<h1 class="header">Elternsprechtag</h1>
			</div>
		</div>
		<a href="" data-reveal-id="MenuModal">
		<div class="row contain-to-grid sticky">
			<div class="twelve columns text-center">
				<span class="" aria-hidden="true" data-icon="&#xe016;" style="font-size:2em;color:#fff;">&nbsp;Menü</span>
			</div>
		</div></a>
		<div class="push"></div>
		<div class="row">
			<!-- CONTENT -->
			<div class="twelve columns">
			<!-- TERMINE -->
				<h2 class="subheader">Ihre Termine</h2>
				<hr>
				<div class="panel">
					<div class="row">
						<!-- Nummer -->
						<div class="one columns">
							<b>#[1]</b>
						</div>
						<!-- Datum & Uhrzeit -->
						<div class="eleven columns">
							<b>Am [25.25.2525] um [25:25]</b>
						</div>
					</div>
					<br>
					<div class="row">
						<div class="one columns">
						</div>
						<!-- Name des Kindes -->
						<div class="four columns">
							<i>Ihr Kind</i><br>
							[Guybrush Threepwood]
						</div>
						<!-- Name der Lehrkraft -->
						<div class="three columns">
							<i>bei</i><br>
							[The Voodoo Lady]
						</div>
						<!-- Fach der Lehrkraft -->
						<div class="four columns">
							<i>im Fach</i><br>
							[Voodoopuppen 101]
						</div>
					</div>
				</div><!-- /TERMIN -->
				<div class="panel">
					<div class="row">
						<!-- Nummer -->
						<div class="one columns">
							<b>#[2]</b>
						</div>
						<!-- Datum & Uhrzeit -->
						<div class="eleven columns">
							<b>Am [25.25.2525] um [25:25]</b>
						</div>
					</div>
					<br>
					<div class="row">
						<div class="one columns">
						</div>
						<!-- Name des Kindes -->
						<div class="four columns">
							<i>Ihr Kind</i><br>
							[Elaine Marley]
						</div>
						<!-- Name der Lehrkraft -->
						<div class="three columns">
							<i>bei</i><br>
							[The Voodoo Lady]
						</div>
						<!-- Fach der Lehrkraft -->
						<div class="four columns">
							<i>im Fach</i><br>
							[Voodoopuppen 101]
						</div>
					</div>
				</div>
				<div class="panel">
					<div class="row">
						<!-- Nummer -->
						<div class="one columns">
							<b>#[3]</b>
						</div>
						<!-- Datum & Uhrzeit -->
						<div class="eleven columns">
							<b>Am [25.25.2525] um [25:25]</b>
						</div>
					</div>
					<br>
					<div class="row">
						<div class="one columns">
						</div>
						<!-- Name des Kindes -->
						<div class="four columns">
							<i>Ihr Kind</i><br>
							[LeChuck]
						</div>
						<!-- Name der Lehrkraft -->
						<div class="three columns">
							<i>bei</i><br>
							[The Voodoo Lady]
						</div>
						<!-- Fach der Lehrkraft -->
						<div class="four columns">
							<i>im Fach</i><br>
							[Voodoopuppen 101]
						</div>
					</div>
				</div>
			</div><!-- /CONTENT -->
		</div><!-- /ROW MAIN LEVEL -->
		<div class="push"></div>
	</div> <!-- /WRAPPER -->
	<!-- FOOTER -->
	<div class="footer"> 
		<div class="row">
			<div class="twelve columns"><hr />
				<div class="row">
					<div class="six columns">
						<p>&copy; Copyright no one at all. Go to town.</p>
					</div>
					<div class="six columns">
						<ul class="link-list right">
							<li><a href="#">Link 1</a></li>
							<li><a href="#">Link 2</a></li>
							<li><a href="#">Link 3</a></li>
							<li><a href="#">Link 4</a></li>
						</ul>
					</div>
				</div>
			</div>
		</div>
	</div> 	<!-- /FOOTER -->
			<!-- MODALS -->
		<div id="MenuModal" class="reveal-modal [small]" style="padding:0;border-radius:5px;">
			<ul class="nav-bar vertical" style="margin-bottom:0;">
				<li class=""><a href=""><span class="nav-icons" aria-hidden="true" data-icon="&#xe00b;">&nbsp;Termine vereinbaren</span></a></li>
				<li class="active"><a href=""><span class="nav-icons" aria-hidden="true" data-icon="&#xe002;">&nbsp;Ihre Termine</span></a></li>
				<li class=""><a href=""><span class="nav-icons" aria-hidden="true" data-icon="&#xe006;">&nbsp;Logout</span></a></li>
			</ul>
			<a class="close-reveal-modal" data-icon="&#xe014;" style="color:#fff;"></a>
		</div>
	<!-- SKRIPTE -->
		<!-- Einbinden der JS Files (Minified) -->
		<script src="js/foundation.min.js"></script>

		<!-- Initialisieren der JS Plugins -->
		<script src="js/app.js"></script>
</body>
</html>
